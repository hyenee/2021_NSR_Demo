...
def main():
    ...
    model_name = st.sidebar.selectbox("Model", ("t5-small", "bart-base", "gpt2-base"))
    ...
    st.sidebar.markdown('- weather.temperature(day.p=오늘,location=부산)')
    st.sidebar.markdown('- weather.uv(day.p=모레,location=\*)')
    st.sidebar.markdown('- weather.general(day.p=\*,location=\*)')
    st.sidebar.markdown('- weather.rain(day.p=내일,location=\*,ti_range.p=\*)')
    st.sidebar.text('')
    st.sidebar.text('')
    st.sidebar.text('')
    st.sidebar.text('')
    ...
    if st.button("Generate"):
        ...
        if not check_exceptions(num_return_sequences):
            ...
            with st.spinner('Progress your text .... '):
                df = pd.read_csv(os.path.join('./data', model_name, "reranking_100.csv"))
